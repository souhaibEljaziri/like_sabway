...
*/
private function generateElements(elementNumber:int, fLocation:float, bStartPatch:boolean):float
{	
...
private function setPrefabHandlers()
{
	...
	{			
		...
		for (var j=0; j<elements[i].tPrefabHandle.Length; j++)//generate obstacle clones
		{
	...
	//Powerups
	var index : int = 0;
	for (i=iObstacleCount; i<(iPowerupCount+iObstacleCount); i++)//traverse through power-ups
	{	
		//(powerupPrefabs[index].GetComponent(PowerupScript) as PowerupScript).enabled = true;
		...
		for (j=0; j<elements[i].tPrefabHandle.Length; j++)//generate clones
		{
			elements[i].tPrefabHandle[j] = Instantiate(powerupPrefabs[index],Vector3(-100,0,0),Quaternion()).transform;
			elements[i].tPrefabHandle[j].parent = tPrefabHandlesParent;
			
			(elements[i].tPrefabHandle[j].GetComponent(PowerupScript) as PowerupScript).enabled = true;
		}//end of inner for
		
		++index;
	}//end of for power-ups
	...
	for (j=0; j<elements[index].tPrefabHandle.Length; j++)//generate clones
	{		
		...
		(elements[index].tPrefabHandle[j].GetComponent(PowerupScript) as PowerupScript).enabled = true;
	}
}