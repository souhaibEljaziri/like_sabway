...
private var aPlayerMecAnim : Animator;
private var mecanimEnabled:boolean=false;
private var v3DefaultPlayerAnimPosition:Vector3;
private var v3DefaultPlayerAnimRotation:Vector3;
...
private var bInStrafe : boolean = false;			//true if the character is strafing
...
private var fStartingWalkSpeed : float = 120.0;//when player starts running
private var fEndingWalkSpeed : float = 200.0;	//final speed after acclerating
...
function Start()
{
	...
	hMissionsController = this.GetComponent(MissionsController) as MissionsController;
	hGlobalAchievementController = this.GetComponent(GlobalAchievementController) as GlobalAchievementController;	
	...
	{
		mecanimEnabled = true;
		aPlayerMecAnim = this.transform.Find("PlayerRotation/PlayerMesh/Prisoner(MecAnim)").GetComponent(Animator) as Animator;
...
function FixedUpdate()
{
	...
	{		
		...
		hMissionsController.incrementMissionCount(MissionTypes.Jump);//count jumps for mission script
		hGlobalAchievementController.incrementAchievementCount(GlobalAchievementTypes.Jump);//count jumps for global achievements script
	}
...
function DeathScene() : boolean
{
	...
	if (iDeathAnimStartTime == 0)
	{
		...
		iDeathAnimStartTime = Time.time;//check game over start time
	}	
...
function strafePlayer(strafeDirection : StrafeDirection)
{
	if (!mecanimEnabled)
	{
		var anim:String = strafeDirection == StrafeDirection.Strafe_Right ? "right" : "left";
		if (isInAir())
		{	
		...
		else if (aPlayer.IsPlaying(anim))	//if strafed while already strafing
		{
			...
			aPlayer[anim].speed = 1.75;
			aPlayer.CrossFade(anim,0.01);
	...
	{
		anim = strafeDirection == StrafeDirection.Strafe_Right ? "StrafeRightAnim" : "StrafeLeftAnim";
		if (isInAir())
		{	
...
public function isMechAnimEnabled() { return mecanimEnabled; }