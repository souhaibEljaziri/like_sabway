...
public function collectedPowerup(index : int)
{
	if(index==PowerUps.Currency)//if a currency unit is collected
	{
		...
		hMissionsController.incrementMissionCount(MissionTypes.Currency);//count the collected currency for mission script
		//count the collected currency for achievement script
		hGlobalAchievementController.incrementAchievementCount(GlobalAchievementTypes.Currency);
	...
	hMissionsController.incrementMissionCount(MissionTypes.Powerups);//count the collected powerups for mission script
	//count the collected powerups for global achievements script
	hGlobalAchievementController.incrementAchievementCount(GlobalAchievementTypes.Powerups);
}
...
private function activatePowerUp(index : int)
{
	...
	else if (index == PowerUps.Ghost)
	{
		hPlayerFrontColliderScript.deactivateCollider();
		hPlayerSidesColliderScript.deactivateCollider();
	}
}
...
*/
public function upgradePowerup(powerupIndex:PowerUps)
{
	fPowerupTotalDuration[powerupIndex] += 3;//increase the duraiton of the power-up by 3 seconds
}