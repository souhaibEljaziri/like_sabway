...
private var iMainMenuButtonsCount:int = 6;	//total number of buttons on main menu
//Pause Menu
...
private var tmPauseMenuMissionList:TextMesh;//mission description on the pause menu
//Game Over Menu
...
//missions menu
private var tMissionsMenuButtons:Transform[];
private var iMissionsMenuButtonsCount:int=1;
private var tmMissionsMenuMissionList:TextMesh;
//achievements menu
private var tAchievementMenuButtons:Transform[];
private var iAchievementMenuButtonsCount:int=1;
private var tmAchievementsMenuDescription:TextMesh;
...
function Start ()
{
	...
	tMainMenuButtons[3] = tMenuTransforms[MenuIDs.MainMenu].Find("Buttons/Button_Shop");
	tMainMenuButtons[4] = tMenuTransforms[MenuIDs.MainMenu].Find("Buttons/Button_Missions");
	tMainMenuButtons[5] = tMenuTransforms[MenuIDs.MainMenu].Find("Buttons/Button_Achievements");
	...
	tmPauseMenuMissionList = tMenuTransforms[MenuIDs.PauseMenu].Find("Text_MissionDescription").GetComponent("TextMesh") as TextMesh;
	...
	//shop
	tMenuTransforms[MenuIDs.Shop] = tMenuGroup.Find("Shop").GetComponent(Transform) as Transform;
	hShopScript = tMenuTransforms[MenuIDs.Shop].GetComponent(ShopScript) as ShopScript;
		
	//missions menu
	tMenuTransforms[MenuIDs.MissionsMenu] = tMenuGroup.Find("MissionsMenu").GetComponent(Transform) as Transform;
	tMissionsMenuButtons = new Transform[iMissionsMenuButtonsCount];
	tMissionsMenuButtons[0] = tMenuTransforms[MenuIDs.MissionsMenu].Find("Buttons/Button_Back").GetComponent(Transform) as Transform;
	tmMissionsMenuMissionList = tMenuTransforms[MenuIDs.MissionsMenu].Find("Text_MissionDescription").GetComponent("TextMesh") as TextMesh;
	
	//achievements menu
	tMenuTransforms[MenuIDs.AchievementsMenu] = tMenuGroup.Find("AchievementsMenu").GetComponent(Transform) as Transform;
	tAchievementMenuButtons = new Transform[iAchievementMenuButtonsCount];
	tAchievementMenuButtons[0] = tMenuTransforms[MenuIDs.AchievementsMenu].Find("Buttons/Button_Back").GetComponent(Transform) as Transform;
	tmAchievementsMenuDescription = tMenuTransforms[MenuIDs.AchievementsMenu].Find("Text_Achievements").GetComponent("TextMesh") as TextMesh;
...
private function listenerClicks()
{
	...
	else if (iTapState == 1)//call relevent handler
	{
		if (Physics.Raycast(HUDCamera.ScreenPointToRay(Input.mousePosition),hit))//if a button has been tapped
		{
			...
			else if (CurrentMenu == MenuIDs.MissionsMenu)
				handlerMissionsMenu(hit.transform);
			else if (CurrentMenu == MenuIDs.AchievementsMenu)
...
*	CALLED BY:	listenerClicks()
*/
private function handlerMissionsMenu(buttonTransform:Transform)
{
	if (tMissionsMenuButtons[0] == buttonTransform)
	{
		CloseMenu(MenuIDs.MissionsMenu);
		ShowMenu(MenuIDs.MainMenu);
	}
}
...
*	CALLED BY:	listenerClicks()
*/
private function handlerAchievementsMenu(buttonTransform:Transform)
{
	if (tAchievementMenuButtons[0] == buttonTransform)
	{
		CloseMenu(MenuIDs.AchievementsMenu);
		ShowMenu(MenuIDs.MainMenu);
	}
}
...
public function ShowMenu(index : int)
{
	...
	else if (MenuIDs.MissionsMenu == index)//display mission menu
	{
		hMissionsController.updateMenuDescriptions();//list the mission on the missions menu
		tMenuTransforms[MenuIDs.MissionsMenu].position.y = 0;//move the missions menu in from of the HUD camera
	}
	else if (MenuIDs.AchievementsMenu == index)
		tMenuTransforms[MenuIDs.AchievementsMenu].position.y = 0;//move the achievements menu in front of the HUD camera
...
private function CloseMenu(index : int)
{
	...
	else if (MenuIDs.MissionsMenu == index)
		tMenuTransforms[MenuIDs.MissionsMenu].position.y = 1000;
	else if (MenuIDs.AchievementsMenu == index)
		tMenuTransforms[MenuIDs.AchievementsMenu].position.y = 1000;
...
*/
public function updatePauseMenuMissions(description:String)
{
...
*/
public function updateMissionsMenuMissions(description:String)
{
...
*/
public function updateAchievementsMenuDescription(description:String)
{
